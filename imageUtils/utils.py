"""
Downloading Cortical NeuroImaging datasets: utility functions
@author: maedbhking
source: Nilearn
...
def get_data_dirs(data_dir=None):
    """Returns the directories in which imageUtils looks for data.
    ...
    3. the global environment variable imageUtils_SHARED_DATA
    4. the user environment variable imageUtils_DATA
    5. imageUtils_data in the user home folder
    """
    ...
    if data_dir is None:
        global_data = os.getenv('imageUtils_SHARED_DATA')
        if global_data is not None:
            paths.extend(global_data.split(os.pathsep))

        local_data = os.getenv('imageUtils_DATA')
        if local_data is not None:
            paths.extend(local_data.split(os.pathsep))

        paths.append(os.path.expanduser('~/imageUtils_data'))
    return paths
...
def _get_dataset_dir(dataset_name, data_dir=None, default_paths=None,
    ...
    raise OSError('imageUtils tried to store the dataset in the following '
